...
scheduler.attachEvent("onBeforeSectionRender", function (render_name, y_unit, timeline){
	...
	if(render_name == "tree"){
		...
		if(y_unit.children) {
			...
			td_className += " folder";
			div_expand = "<div class='dhx_scell_expand'>"+((y_unit.open)?'<span class="glyphicon glyphicon-minus-sign"></span>':'<span class="glyphicon glyphicon-plus-sign"></span>')+"</div>";
			table_className = (timeline.folder_events_available)?"dhx_data_table folder_events":"dhx_data_table folder";			
			suffix_icons = " <span class='badge'>"+y_unit.children.length+"</span>";
			...
			prefix_icons = "<span class='glyphicon glyphicon-user'></span> ";
		}
		...
		var td_content = "<div class='dhx_scell_level"+y_unit.level+"'>"+div_expand+"<div class='dhx_scell_name'>"+prefix_icons+(scheduler.templates[timeline.name+'_scale_label'](y_unit.key, y_unit.label, y_unit)||y_unit.label)+suffix_icons+"</div></div>";
